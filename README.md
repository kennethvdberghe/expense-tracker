...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create expense-tracker-c026
  fly apps create expense-tracker-c026-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app expense-tracker-c026
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app expense-tracker-c026-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'expense-tracker-c026-staging' does not match app name in config file 'expense-tracker-c026'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name expense-tracker-c026-db
  fly postgres attach --app expense-tracker-c026 expense-tracker-c026-db

  fly postgres create --name expense-tracker-c026-staging-db
  fly postgres attach --app expense-tracker-c026-staging expense-tracker-c026-staging-db
  ```
